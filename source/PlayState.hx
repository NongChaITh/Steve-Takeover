...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'ugh':
				dialogue = CoolUtil.coolTextFile(Paths.txt('ugh/steveDia'));
		}
			...
			case 'tankStage': 
				{
					curStage = 'tankStage';
					defaultCamZoom = 1;
					var bg:FlxSprite = new FlxSprite(-400, -345);
					bg.loadGraphic(Paths.image("BG/tankSky", 'shared'));
					bg.scrollFactor.set(0, 0);
					bg.antialiasing = true;
					//bg.setGraphicSize(Std.int(bg.width * 1.5));
					add(bg);

					var clouds:FlxSprite = new FlxSprite(FlxG.random.int(-700, -100), FlxG.random.int(-20, 20)).loadGraphic(Paths.image('BG/tankClouds', 'shared'));
					...
					var ruins:FlxSprite = new FlxSprite(-200,0).loadGraphic(Paths.image('BG/tankRuinandBuilding', 'shared'));
					...
					var ground:FlxSprite = new FlxSprite(-315,-150).loadGraphic(Paths.image('BG/tankBridge', 'shared'));
					...
					tankCrowd = new FlxSprite(40, 55);
					tankCrowd.frames = Paths.getSparrowAtlas('BG/bg', 'shared');
					tankCrowd.animation.addByPrefix('tankIdle', 'bg tank idle', 24, false);
					tankCrowd.antialiasing = true;
					tankCrowd.scrollFactor.set(0.9, 0.9);
					tankCrowd.setGraphicSize(Std.int(tankCrowd.width * 1));
					tankCrowd.updateHitbox();
					if(FlxG.save.data.distractions){
						add(tankCrowd);
					}

					crowd = new FlxSprite(390, 825);
					crowd.frames = Paths.getSparrowAtlas('BG/crowd', 'shared');
					crowd.animation.addByPrefix('crowds', 'crowd idle', 24, false);
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						switch (Math.abs(daNote.noteData))
						{
							case 2:
								...
								if(SONG.player2 == 'steve-crazy' || SONG.player2 == 'steve')
									{
										health -= 0.01;
									}
							case 3:
	...
	*/
	...
	override function stepHit()
		...
		/*if (dad.curCharacter == 'steve' && SONG.song.toLowerCase() == 'ugh')
			{
					
				if (curStep == 59 || curStep == 443 || curStep == 523 || curStep == 827) // -1
				{
					dad.addOffset("singUP", 10, -52);
						
					dad.animation.getByName('singUP').frames = dad.animation.getByName('Ahh').frames;
				}
		
				if (curStep == 64 || curStep == 448 || curStep == 528 || curStep == 832) // +4
				{
					dad.addOffset("singUP", 10, -52);
					dad.animation.getByName('singUP').frames = dad.animation.getByName('singUPP').frames;
				}
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'tankStage':
				if(FlxG.save.data.distractions){
					tankCrowd.animation.play('tankIdle', true);
					crowd.animation.play('crowds', true);
				}
